<template>
    ...
    <select v-model="NewStudent.vitri">
        ...
        <option v-for="(lop,index) in props.infor" :key="index" :value="index">{{ lop.lop }}</option>    
    </select><br>
...
const props=defineProps(['infor','themhocsinh'])
...
const Luu=()=>{
    if(NewStudent.vitri!==null&&NewStudent.name!==''&&NewStudent.age!==null){
        props.themhocsinh(NewStudent.vitri,NewStudent.name,NewStudent.age);
